...
liste = []
for i in data:
  if(count not in [ 10, 11, 13, 14, 15, 28, 29, 31, 32, 34, 35 , 36 , 37, 38, 39, 40,41, 43, 50, 51, 54, 55, 56 ]):
    liste.append(i)
...
"""
##KNeighborsClassifier 
Nous avons remarqué qu'en excluant les colonnes qui selon notre étude de corrélation précédente n'avais que très peu d'impact sur nos prédictions, la classification KNN s'est améliorée en passant à 84% de précision au lieu de 78%.
"""
...
"""# Clustering
A priori enlever les colonnes en questions n'a pas aidé ce modèle qui à plus ou moins stagné.
"""
...
En revanche la regression linéaire semble arrivé à une précision de 100%, ce qui est théoriquement impossible. Cela doit être dû au fait que le dataset à été consu sur-mesure en analysant la présence de mot stratégique étant donné qu'on n'analyse que les mails d'une seule personne.
"""
...
"""Etant donné que ce model est le plus precis nous l'avons exporter grâce au module pickle pour l'intégré dans notre api."""
